...
#include <seastar/parquet/parquet/column_scanner.h>
...
class TestAllTypesPlain : public ::testing::Test {
 public:
  void SetUp() {
    reader_ = seastarized::ParquetFileReader::Open(
      seastarized::RandomAccessFile::Open(alltypes_plain()).get0()).get0();
  }

  void TearDown() {}

 protected:
  std::unique_ptr<seastarized::ParquetFileReader> reader_;
};

TEST_F(TestAllTypesPlain, NoopConstructDestruct) {}

TEST_F(TestAllTypesPlain, TestBatchRead) {
  std::shared_ptr<seastarized::RowGroupReader> group = reader_->RowGroup(0);

  // column 0, id
  std::shared_ptr<seastarized::Int32Reader> col =
    std::dynamic_pointer_cast<seastarized::Int32Reader>(group->Column(0));
  ...
  ASSERT_TRUE(col->HasNext().get0());
  levels_read = col->ReadBatch(5, def_levels, rep_levels, values, &values_read).get0();
  ASSERT_EQ(4, levels_read);
  ASSERT_EQ(4, values_read);
...
TEST_F(TestAllTypesPlain, TestFlatScannerInt32) {
  ...
  // column 0, id
  auto scanner = std::make_shared<seastarized::Int32Scanner>(group->Column(0));
  int32_t val;
  bool is_null;
  for (int i = 0; i < 8; ++i) {
...
TEST_F(TestAllTypesPlain, TestSetScannerBatchSize) {
  ...
  // column 0, id
  std::shared_ptr<seastarized::Int32Scanner> scanner(
    new seastarized::Int32Scanner(group->Column(0)));
...
TEST_F(TestAllTypesPlain, DebugPrintWorks) {
  ...
  seastarized::ParquetFilePrinter printer(reader_.get());
...
TEST_F(TestAllTypesPlain, ColumnSelectionOutOfRange) {
  ...
  seastarized::ParquetFilePrinter printer1(reader_.get());
  ASSERT_THROW(printer1.DebugPrint(ss, columns), ParquetException);

  columns.clear();
  columns.push_back(-1);
  seastarized::ParquetFilePrinter printer2(reader_.get());
...
class TestLocalFile : public ::testing::Test {
 public:
  void SetUp() {
    ...
    handle = seastarized::RandomAccessFile::Open(ss.str()).get0();
  }

  void TearDown() {}

 protected:
  std::shared_ptr<seastarized::RandomAccessFile> handle;
};

TEST_F(TestLocalFile, OpenWithMetadata) {
  // PARQUET-808
  std::stringstream ss;
  std::shared_ptr<FileMetaData> metadata = seastarized::ReadMetaData(handle).get0();

  auto reader = seastarized::ParquetFileReader::Open(
    handle, default_reader_properties(), metadata).get0();

  // Compare pointers
  ASSERT_EQ(metadata.get(), reader->metadata().get());

  std::list<int> columns;
  seastarized::ParquetFilePrinter printer(reader.get());
  printer.DebugPrint(ss, columns, true);

  // Make sure OpenFile passes on the external metadata, too
  auto handle2 = seastarized::RandomAccessFile::Open(alltypes_plain()).get0();
  auto reader2 = seastarized::ParquetFileReader::Open(
    handle2, default_reader_properties(), metadata).get0();
...
Column 1
  ...
  auto source = seastarized::RandomAccessFile::Open(data_file(file)).get0();
  auto reader = seastarized::ParquetFileReader::Open(source).get0();
  seastarized::ParquetFilePrinter printer(reader.get());
...
TEST(TestJSONWithLocalFile, JSONOutput) {
  ...
  auto source = seastarized::RandomAccessFile::Open(alltypes_plain()).get0();
  auto reader = seastarized::ParquetFileReader::Open(source).get0();
  seastarized::ParquetFilePrinter printer(reader.get());
...
TEST(TestFileReader, BufferedReads) {
  ...
  std::shared_ptr<arrow::io::BufferOutputStream> out_sink;
  ASSERT_OK(arrow::io::BufferOutputStream::Create(1024, arrow::default_memory_pool(),
                                                  &out_sink));
  auto out_stream = std::make_shared<seastarized::MemoryFutureOutputStream>(out_sink);
  std::shared_ptr<seastarized::ParquetFileWriter> file_writer =
    seastarized::ParquetFileWriter::Open(out_stream, schema, writer_props).get0();

  seastarized::RowGroupWriter* rg_writer = file_writer->AppendRowGroup().get0();
  ...
  for (int col_index = 0; col_index < num_columns; ++col_index) {
    seastarized::DoubleWriter* writer =
      static_cast<seastarized::DoubleWriter*>(rg_writer->NextColumn().get0());
    std::shared_ptr<arrow::Array> col = rag.Float64(num_rows, 0, 100);
    const auto& col_typed = static_cast<const ::arrow::DoubleArray&>(*col);
    writer->WriteBatch(num_rows, nullptr, nullptr, col_typed.raw_values()).get0();
  ...
  rg_writer->Close().get0();
  file_writer->Close().get0();

  // Open the reader
  std::shared_ptr<Buffer> file_buf;
  ASSERT_OK(out_stream->Finish(&file_buf));
  auto in_file = std::make_shared<seastarized::RandomAccessBuffer>(file_buf);

  ReaderProperties reader_props;
  reader_props.enable_buffered_stream();
  reader_props.set_buffer_size(64);
  std::unique_ptr<seastarized::ParquetFileReader> file_reader =
    seastarized::ParquetFileReader::Open(in_file, reader_props).get0();

  auto row_group = file_reader->RowGroup(0);
  std::vector<std::shared_ptr<seastarized::DoubleReader>> col_readers;
  for (int col_index = 0; col_index < num_columns; ++col_index) {
    col_readers.push_back(
      std::static_pointer_cast<seastarized::DoubleReader>(row_group->Column(col_index)));
  }
  ...
  for (int row_index = 0; row_index < num_rows; ++row_index) {
    for (int col_index = 0; col_index < num_columns; ++col_index) {
      ...
      int64_t levels_read =
        col_readers[col_index]->ReadBatch(1, nullptr, nullptr, out, &values_read).get0();

...
public:
    ...
    void append_value(schema::logical_type::TIME_INT32 t, int32_t v) { print_time(static_cast<uint64_t>(v), 1000, 3); }
    void append_value(schema::logical_type::TIME_INT64 t, int64_t v) {
        if (t.unit == t.MICROS) {
...
cql_schema parquet_schema_to_cql_schema(const schema::schema& parquet_schema, const std::string& table) {
    int udt_index = 0;
    auto convert = y_combinator{[&] (auto&& convert, const schema::node& parquet_node) -> node {
        return std::visit(overloaded {

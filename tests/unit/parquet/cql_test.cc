...
CREATE TABLE parquet ("pk" timeuuid PRIMARY KEY, "a" frozen<map<text, frozen<map<int, boolean>>>>, "b" int, "c" double);
{a: {"a": {1: 1, 2: 0}}, b: 1, c: 1}
{a: {"b": {1: 1}}, b: 1, c: 1}
{a: {"c": null}, b: 1, c: 1}
{a: {"d": {}}, b: 1, c: 1}
...
CREATE TABLE parquet ("pk" timeuuid PRIMARY KEY, "a" frozen<list<frozen<list<frozen<list<text>>>>>>, "b" int);
{a: [[["a", "b"], ["c"]], [null, ["d"]]], b: 1}
{a: [[["a", "b"], ["c", "d"]], [null, ["e"]]], b: 1}
{a: [[["a", "b"], ["c", "d"], ["e"]], [null, ["f"]]], b: 1}
)###"},
        {"alltypes_plain.snappy.parquet", R"###(
CREATE TABLE parquet ("pk" timeuuid PRIMARY KEY, "id" int, "bool_col" boolean, "tinyint_col" int, "smallint_col" int, "int_col" int, "bigint_col" bigint, "float_col" float, "double_col" double, "date_string_col" blob, "string_col" blob, "timestamp_col" varint);
...
CREATE TYPE utd_1(phone: frozen<list<udt_0>>);
CREATE TABLE parquet ("pk" timeuuid PRIMARY KEY, "id" int, "phoneNumbers" udt_1);
{id: 1}
{id: 2}
...
CREATE TABLE parquet ("pk" timeuuid PRIMARY KEY, "value" decimal);
{value: 100}
{value: 200}
{value: 300}
{value: 400}
...
{value: 2400}
        ...
        for (const auto& [filename, output] : test_cases) {
            ...
            auto record_reader = cql::record_reader::make(reader, 0).get0();
            cql::cql_consumer cc{ss};
            record_reader.read_all<cql::cql_consumer>(cc).get();

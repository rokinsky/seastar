...
SEASTAR_TEST_CASE(full_roundtrip) {
    ...
    return seastar::async([] {
        // Write
        write_schema::schema write_schema = [] () -> write_schema::schema {
            using namespace write_schema;
            return write_schema::schema{vec<node>(
                map_node {"Map", true,
                    box<node>(primitive_node {"Map key", false, schema::logical_type::STRING{}}),
                    box<node>(primitive_node {"Map value", false, schema::logical_type::INT32{}}),
                },
                list_node {"List", true,
                    box<node>(struct_node {"Struct", true, vec<node>(
                        primitive_node {"Struct field 1", false, schema::logical_type::FLOAT{}},
                        primitive_node {"Struct field 2", false, schema::logical_type::DOUBLE{}}
                    )})
                }
            )};
        }();

        std::unique_ptr<file_writer> fw = file_writer::open(test_file_name, write_schema).get0();
